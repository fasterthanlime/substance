...
use crate::reporting::{LlvmFunction, Symbol};
use crate::types::{ByteSize, CrateName, DemangledSymbol, LlvmFunctionName, MangledSymbol};
...
pub struct BuildContext {
    ...
    pub global_timing: Option<Duration>,
}
...
pub struct AnalysisResult {
    /// Size of the artifact (full file)
    pub file_size: ByteSize,

    /// Size of the .text sectoin
    pub text_size: ByteSize,

    /// Build times per crate
    pub crate_build_times: HashMap<CrateName, Duration>,

    /// Symbols found in the artifact
...
impl BuildRunner {
    ...
    pub fn run(&self) -> Result<BuildContext, SubstanceError> {
        ...
        let stderr_handle = thread::spawn(move || {
            ...
            for line_result in reader.lines() {
                if let Ok(line) = line_result {
                    // Write line to parent's stderr
                    writeln!(handle, "{}", line).ok();
                    // Do not collect stderr lines.
                }
            }
        });
        ...
        // Try to find the global timing (sum of all durations).
        let global_timing = if !stdout_result.timing_infos.is_empty() {
            let total = stdout_result
                .timing_infos
                .iter()
                .map(|ti| ti.duration)
                .sum::<f64>();
        ...
        let context = BuildContext {
            ...
            dep_crates,
            deps_symbols,
            timing_data: stdout_result.timing_infos.clone(),
            global_timing,
        };

        let analysis_result = AnalysisResult {
            file_size: raw_data.file_size,
            text_size: raw_data.text_size,
            crate_build_times,
        ...
        Ok(BuildResult {
            ...
            json_lines: Vec::new(), // original JSON lines are not preserved, can be added if needed
        })
    }
